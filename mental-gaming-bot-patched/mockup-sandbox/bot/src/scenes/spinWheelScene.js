...
const spinWheelScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    const text =
      ...
      `*Prize Pool:*\n` +
      prizeLines.join('\n') +
      `\n\n_Paid spin costs ${SPIN_COST_COINS} Mental Coins._`;
    ...
    if (freeSpin) {
      ...
      buttons.push([Markup.button.callback(`🪙 Paid Spin (${SPIN_COST_COINS} MC)`, 'spin_paid')]);
    }

    if (!freeSpin && hasPaidCoins) {
      // already shown above
    } else if (!freeSpin && !hasPaidCoins) {
      buttons.push([Markup.button.callback(`❌ Not Enough Coins (Need ${SPIN_COST_COINS})`, 'noop')]);
    }
...
async function executeSpin(ctx, usePaidSpin) {
  await ctx.answerCbQuery(usePaidSpin ? `Spending ${SPIN_COST_COINS} coins...` : 'Spinning!');
  ...
  try {
    ...
    const spinTypeLabel = usedFreeSpin ? '🆓 Free Spin' : `🪙 Paid Spin (${SPIN_COST_COINS} MC)`;
    const rewardLines = [];

    if (prize.type === 'ks') {
      rewardLines.push(`💰 +${prize.value.toLocaleString()} KS added to your wallet!`);
