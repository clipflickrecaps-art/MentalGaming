...
async function checkStockWarning(product, telegram) {
  if (product.productType === 'DigitalCode') {
    ...
    if (available <= LOW_STOCK_THRESHOLD && available > 0) {
      try {
    ...
    if (product.stockCount !== -1 && product.stockCount <= LOW_STOCK_THRESHOLD && product.stockCount > 0) {
      try {
        await telegram.sendMessage(
          config.bot.adminId,
          `⚠️ *Low Stock Warning*\n\n` +
...
async function createOrder(telegramId, productId, { gameId = null, zoneId = null, gameName = null, requiredInfo = [], promoCode = null, promoDiscount = 0, tierDiscount = 0, tierDiscountPct = 0, finalAmount = null } = {}) {
