...
async function navigate(ctx, folderId, editMessage = false) {
  const folder = registry.get(folderId);
  if (!folder) {
  ...
  // Always render menus from the latest DB user state. This prevents stale
  // language/balance/profile data after actions like check-in, wallet updates,
  // or settings changes.
  try {
    if (ctx.from?.id) {
      const User = require('../models/User');
      const freshUser = await User.findByTelegramId(ctx.from.id);
      if (freshUser) ctx.user = freshUser;
    }
  } catch (err) {
    console.warn('[Nav] User refresh failed:', err.message);
  }
