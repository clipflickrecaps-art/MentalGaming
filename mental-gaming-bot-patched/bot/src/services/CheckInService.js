 ...
 *   Day 5 →  60 MC
 *   Day 6 →  90 MC
 *   Day 7 → 150 MC + 100 KS  🎉 Weekly Jackpot!
 *
 * Milestone bonuses (on top of daily reward):
 *   14-day streak → +250 MC + 200 KS  🏅 Two-Week Champion
 *   30-day streak → +500 MC + 500 KS  🏆 Monthly Legend
 *
...
const DAILY_REWARD = [
  ...
  { coins: 150, ks: 100, label: 'Day 7 🎉', milestone: true },  // index 6 → day 7
];
...
async function getDayReward(streakDay) {
  const cfg = await GameConfig.get().catch(() => null);
  const rewards = Array.isArray(cfg?.checkInRewards) && cfg.checkInRewards.length >= 7 ? cfg.checkInRewards : DAILY_REWARD;
  const idx = (streakDay - 1) % 7;
  const r = rewards[idx] || DAILY_REWARD[idx];
  return { coins: Number(r.coins || 0), ks: Number(r.ks || 0), label: r.label || `Day ${idx+1}`, milestone: idx === 6 };
}

// ── Check if user can check in today ─────────────────────────────────────────
async function getCheckInStatus(telegramId) {
  const user = await User.findOrCreate(telegramId, null, null);
  if (!user) return null;
  ...
  return {
    ...
    todayDate: today,
    nextReward: await getDayReward((user.checkInStreak || 0) + 1),
  };
}
...
async function doCheckIn(telegramId) {
  ...
  // Get daily reward
  const dayReward = await getDayReward(newStreak);
  ...
  return {
    ...
    user:            updatedUser,
    nextReward:      await getDayReward(newStreak + 1),
  };
}
...
async function getRewardPreview(currentStreak) {
  const lines = [];
  for (let i = 1; i <= 7; i++) {
    const r = await getDayReward(i);
