...
Nav.register({
  ...
  build: async (ctx, theme) => {
    ...
    const keyboard = Markup.inlineKeyboard([
      ...
      [Markup.button.callback('🎰 Spin Rewards',  'admin_spin_full'),       Markup.button.callback('💱 Rates',       'admin_rates_action')],
      [Markup.button.callback('💳 Payments',      'admin_payments_full'),   Markup.button.callback('🗓 Check-In',    'admin_checkin_full')],
      [Markup.button.callback('📢 Broadcast',     'admin_broadcast_action'), Markup.button.callback('📋 Audit Logs',  'admin_audit_action')],
      [Markup.button.callback('📁 Categories',    'cat_manager'), Markup.button.callback('📢 Channels', 'admin_channels_full')],
...
module.exports = function registerAdmin(bot) {
  ...
  bot.command('admin', adminOnly(), async (ctx) => {
    return ctx.reply('🔧 *Admin Panel*\n\nReply keyboard mode enabled. Choose a section below.', {
      parse_mode: 'Markdown',
      ...Markup.keyboard([
    ...
    await ctx.reply(
      `🛍️ *Product Management*\n\n✅ Active: *${active}*\n🔴 Inactive: *${total - active}*\n📦 Total: *${total}*`,
      {
        ...
        ...Markup.inlineKeyboard([
          ...
          [Markup.button.callback('📁 Categories', 'cat_manager'), Markup.button.callback('📦 Bulk Add', 'prod_bulk_add')],
  ...
  bot.action('admin_users_action', adminOnly(), async (ctx) => {
    ...
    await ctx.reply(`👥 *User Management*\n\nChoose an action:`, {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.callback('🔎 Manage User', 'user_find_manage')],
  ...
  bot.action('admin_payments_action', adminOnly(), async (ctx) => {
    await ctx.answerCbQuery();
    const PaymentMethod = require('../models/PaymentMethod');
    const methods = await PaymentMethod.find().sort({ displayOrder: 1, name: 1 });
    const lines = methods.length
      ? methods.map((m, i) => `${i + 1}. ${m.emoji || '💳'} *${m.name}* — \`${m.accountNumber}\` — ${m.isActive ? '🟢 Active' : '🔴 Inactive'}`).join('\n')
      : 'No payment methods configured yet.';
    await ctx.reply(`💳 *Payment Methods*\n\n${lines}`, {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.callback('➕ Add Payment Method', 'addpayment_start')],
        [Markup.button.callback('🔙 Back', 'nav:go:admin_main')],
      ]),
    });
  });

  bot.action('admin_checkin_action', adminOnly(), async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.reply(
      `🗓 *Check-In Rewards*\n\nCurrent rewards are controlled by CheckInService milestones.\n\nNext patch target: add full editable reward rows from admin panel.`,
      { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('🔙 Back', 'nav:go:admin_main')]]) }
    );
  });
