...
async function getCoinBonusRates() {
  ...
  try {
    ...
    if (cfg.customTiers?.length) {
      _ratesCache = Object.fromEntries(cfg.customTiers.map((t) => [t.name, Number(t.bonusRate || 0)]));
      if (!_ratesCache.Silver) _ratesCache.Silver = 0.01;
    } else {
      _ratesCache = {
...
async function creditKS(userId, amount, { type = 'AdminCredit', note = '', paymentMethod = null, screenshotUrl = null, txId = null } = {}) {
  ...
  try {
    ...
    if (type === 'Topup') {
      ...
      try {
        const { calcTierFromDeposited } = require('./MembershipService');
        user.membershipTier = await calcTierFromDeposited(user.totalDeposited);
      } catch {
        user.recalcTier();
      }
    }
