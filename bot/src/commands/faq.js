...
function faqDetailKeyboard(faq) {
  ...
  buttons.push([Markup.button.callback('🔙 Back to Main', 'faq_back_main')]);
...
module.exports = function registerFAQ(bot) {
    ...
    await ctx.reply(
      ...
      `_e.g. /faq how to topup_`,
      {
        ...
        ...Markup.inlineKeyboard([
          ...
          [Markup.button.callback('🔙 Back to Main', 'faq_back_main')],
        ]),
      }
    );
  });
  ...
  bot.action('faq_search_prompt', async (ctx) => {
    ...
    await ctx.reply(
      ...
      { parse_mode: 'Markdown', ...faqReplyKeyboard() }
    );
  });
  ...
  bot.action('faq_back_main', async (ctx) => {
    await ctx.answerCbQuery();
  ...
  bot.hears('📚 FAQ', async (ctx) => ctx.reply('Type /faq to browse FAQ categories.', faqReplyKeyboard()));
